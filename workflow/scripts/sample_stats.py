...
# write headers
output.write("sample\ttotal_reads\tmapped_reads\tdedup_reads\tdedup_pct\tmean_coverage\tat_dropout\tgc_dropout\n")
...
for file in os.listdir("results/bwa_mem/"):
    if file.endswith(".bam"):
        ...
        atdrop = subprocess.run(["grep", "-A", "1", "AT_DROPOUT", f"results/gcbias/{sample}.summary.txt"], encoding='utf-8', stdout=subprocess.PIPE).stdout.split("\n")[1].split("\t")[5]
        gcdrop = subprocess.run(["grep", "-A", "1", "GC_DROPOUT", f"results/gcbias/{sample}.summary.txt"], encoding='utf-8', stdout=subprocess.PIPE).stdout.split("\n")[1].split("\t")[6]

        # write stats
        output.write(f"{sample}\t{total_reads}\t{mapped_reads}\t{dedup_reads}\t{dedup_pct}\t{mean_coverage}\t{atdrop}\t{gcdrop}\n")
